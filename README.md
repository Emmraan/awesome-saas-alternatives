...
docs/
  handoffs/          HANDOFF-00 … HANDOFF-13 (per-phase implementation log)
  reference/         product-source-list.md (canonical 131-row source table)
.github/workflows/
  validate.yml       PR checks: install → lint → typecheck → test:coverage → validate-data
  release.yml        Push-to-main → semantic-release
```

Ownership per phase is documented in [PLAN.md](PLAN.md) (File ownership map).
...
Coverage is gated at **≥ 80%** on `lib/` (statements, branches, functions, lines). The PR workflow [`validate.yml`](.github/workflows/validate.yml) enforces `test:coverage` — PRs cannot merge below the gate. See [PLAN.md P12](PLAN.md#p12--tests--quality-gate).
...
What’s shipped and what’s next — mirrors [PLAN.md](PLAN.md).
...
- **Next (P15):** `AGENTIC_BUILD_GUIDE.md` — the reusable agentic build playbook (intake → skills → phases → branches → handoffs → maintenance) distilled from this repo.
...
- Product inventory seeded from the canonical table in [`docs/reference/product-source-list.md`](docs/reference/product-source-list.md) (131 paid rows, ~60 unique alternatives).
- Built with the `.agents/skills` skill system and a branch-per-phase workflow documented in [PLAN.md](PLAN.md) — each phase ships via a squash-merge with one Conventional Commit.
